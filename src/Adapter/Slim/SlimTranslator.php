...
/**
 * Slim 没有翻译组件，这里直接读 `{目录}/{语种}/messages.php`（就是包的 translations/ 结构），
 * 用 symfony/translation 反而会多一层「未命中返回什么」的不确定性 —— 而这一点正是内核依赖的契约。
 *
 * 两条硬性语义：
 *  - **未命中返回 key 本身**。UploadController::fail() 靠「异常消息 == 翻译 key」判定错误能否对外暴露；
 *    这里刻意**不做 fallback**（不回落 en）：命中不了就是未命中，[6] 号集成断言就靠这一点成立。
 *  - **setLocale 不得跨请求泄漏**：语种存在 RequestContext 里，执行上下文（请求）一换就复位。
 */
class SlimTranslator implements TranslatorInterface
{
    /**
    ...
    {
        // 语种是「本次请求/协程」的状态，必须落在 RequestContext（webman 常驻进程与协程宿主下
        // 挂在适配器上会被下一个请求覆盖）
