...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 【一般设置】秒传功能在Redis中保存的hash与存储路径对应记录的有效期，到期后自动过期。默认7天(604800秒)。
    |
    */
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | nginx内部重定向(X-Accel-Redirect)
    |--------------------------------------------------------------------------
    |
    | 【一般设置】默认不启用，仅在nginx部署下使用。webman的文件响应不实现HTTP Range，大文件下载会整包下发，
    | 视频无法拖动进度条，且整个传输期间占用一个worker进程；开启后改由nginx直接发送文件，worker立即释放，
    | 并支持Range(断点续传、视频拖动)。
    |
    | 需在nginx中为该内部前缀配置location，alias指向项目上传根目录的绝对路径(注意结尾的斜杠)：
    |
    ...
    | }
    |
    | 修改root_dir后alias需同步调整；该location必须保留internal，防止外部绕过程序直接访问资源文件。
    |
    */
    ...
    */
    ...
    'groups' => [
        ...
        'file' => [ # 分组名
            ...
            'resource_maxsize'             => 104857600, # 被允许的资源文件最大值(B)，0为不限制，32位系统最大值为2147483647；默认104857600(100MB)，如需不限制请改回0
            'resource_extensions'          => ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'txt', 'zip', 'rar', '7z', 'mp4', 'mp3', 'wav'], # 被允许的资源文件扩展名(白名单)，空为不限制；默认白名单仅放行常见图片/文档/音视频，如需其他类型请自行添加。注意：此默认值改变了原"空=不限制"的行为，升级前请确认业务所需类型
